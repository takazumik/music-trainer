...
export default function Home() {
  ...
  useEffect(() => {
    handleReset();
  }, [settings.key, settings.scaleType, handleReset]);

  // MIDI接続
  const { midiStatus, selectedDeviceId, selectDevice } = useMidi(
    handleNotePress,
    handleNoteRelease
  );
    ...
    <main className='min-h-screen bg-gradient-to-b from-blue-50 to-blue-100 py-8'>
      <div className='container mx-auto px-4'>
        ...
        <header className='text-center mb-8'>
          ...
          <p className='text-gray-600'>スケールとコードを楽しく学ぼう！</p>
          {completedScales > 0 && (
        ...
        {midiStatus.isSupported && (
          <div className='text-center mb-4'>
            <p
              className={`text-sm ${
                midiStatus.isConnected ? "text-green-600" : "text-gray-500"
              }`}
            >
              MIDI:{" "}
              {midiStatus.isConnected
                ? `接続済み (${midiStatus.deviceName})`
                : "未接続"}
            </p>
          </div>
        )}
        ...
        <div className='text-center mb-6'>
          <h3 className='text-lg font-semibold text-gray-700'>
            {settings.key}{" "}
            {settings.scaleType === "major" ? "メジャー" : "マイナー"}スケール
          </h3>
